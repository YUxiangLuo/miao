...
- **桌面双列（>1180px）位置钉死、高度随视口伸缩**：融合顶栏 76（64px logo/速率/模式/版本一条卡片，与内容同宽对齐；品牌文字与 Sing-box 状态块已移除，body 上下 padding 6px 保持总高不变）/ 内容网格 `max(582, 100vh−388)`（388=工作区上下 padding 96（各 48，上下留白一致）+顶栏 76+16+条带 16+184）/ 条带 184；右列三卡按 156:196:246 等比分配（630 设计高度时恰为 156/196/246），min-height 钉住设计值，网格低于 630 时整列内部滚动兜底。右列固定卡的滚动在 `.panel-card-body` 内部（header 固定）。841–1180px 单列堆叠带保持弹性
