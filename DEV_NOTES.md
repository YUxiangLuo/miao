...
TUN JSON：显式 `stack: "go"`（固定上游已默认使用 GoTUN）、`auto_route` + `strict_route`，`interface_name` 仍是 `sing-tun`。`cfg(target_os = "linux")` 才写 `auto_redirect`。
...
启动先按兼容缓存 → 订阅节点快照 → 手动节点尝试本地启核，再在锁外恢复订阅；本地材料不足时才拉取后启核。数据面已可用时，后台订阅刷新首轮最多 20s，随后最多 4 次快速单次重试（5/10/20/40s 退避）；仍失败则保留当前运行配置、显示明确告警，改为每 30 分钟静默重试，不再反复切换面板启动状态。停服/改订阅及时取消长等待，前台失败不重置快速重试额度；数据面本身未就绪的恢复仍使用原有持续退避。自升级健康点在内嵌文件释放成功且预期数据面 ready 后；空配置无需数据面，用户明确停服也视为运行状态已稳定。
