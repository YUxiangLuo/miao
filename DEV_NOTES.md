...
- **桌面双列（>1180px）位置钉死、高度随视口伸缩**：状态卡 76 / 内容网格 `max(630, 100vh−404)`（404=topbar 60+工作区 padding 48+状态卡 76+其间距 20+条带上间距 16+条带 184）/ 条带 184；右列三卡按 156:196:246 等比分配（630 设计高度时恰为 156/196/246），min-height 钉住设计值——高视口零留白，偏矮时收缩失效退回列内滚动兜底。右列固定卡的滚动在 `.panel-card-body` 内部（header 固定）。841–1180px 单列堆叠带保持弹性
