...
| 网络地图 | 世界地图上可视化本机位置、代理节点与全部连接的地理分布（本地 GeoIP 数据库，直连/代理分色，Esc 退出） |
...
location: "31.23,121.47"   # 可选:本机位置手动覆盖(地图模式),不设置则自动探测真实出口 IP
...
adblock: true              # 可选:去广告(内嵌规则集,路由层拦截,默认关闭)
```
