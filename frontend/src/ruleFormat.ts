...
export const RULE_FIELD_OPTIONS: RuleFieldOption[] = [
...
export function ruleFieldOptions(platform = 'linux'): RuleFieldOption[] {
  if (platform !== 'windows') {
...
export const PROTOCOL_OPTIONS: RuleTargetOption[] = [
...
export function ruleFieldTone(field: string): BadgeTone {
  return RULE_FIELD_TONES[field] || 'neutral'
}

export function ruleTargetLabel(target: string | null | undefined): string {
  return RULE_TARGET_OPTIONS.find((option) => option.value === target)?.label || target || '未知'
}

/** describeRule 的结构化分支 */
export interface StructuredRuleDescription {
  structured: true
  field: string
  fieldLabel: string
  value: string
...
/** describeRule 的兜底分支：手写任意 JSON 规则 */
export interface RawRuleDescription {
  structured: false
  raw: string
}

export type RuleDescription = StructuredRuleDescription | RawRuleDescription

// 规范化后端返回的 RuleInfo;手写规则可能没有 target,调用方不得臆造
export function describeRule(info: RuleInfo | null | undefined): RuleDescription {
