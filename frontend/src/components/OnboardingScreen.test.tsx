...
function renderOnboarding(props = {}) {
  return render(
    <OnboardingScreen
      ...
      onOpenAddNode={vi.fn()}
      showToast={vi.fn()}
      onScanVerge={vi.fn().mockResolvedValue(null)}
      onImportVerge={vi.fn().mockResolvedValue(true)}
      {...props}
    />
  )
}
...
describe('OnboardingScreen', () => {
  ...
  it('informs when no Clash Verge subscriptions are detected', async () => {
    const user = userEvent.setup()
    const onScanVerge = vi.fn().mockResolvedValue({ found: false, items: [] })
    const showToast = vi.fn()
  ...
  it('informs when everything detected is already added', async () => {
    const user = userEvent.setup()
    const onScanVerge = vi.fn().mockResolvedValue({
      found: true,
  ...
  it('imports only the selected subscriptions', async () => {
    ...
    const onScanVerge = vi.fn().mockResolvedValue({ found: true, items })
    const onImportVerge = vi.fn().mockResolvedValue(true)
  ...
  it('collapses the panel on cancel without importing', async () => {
    const user = userEvent.setup()
    const onScanVerge = vi.fn().mockResolvedValue({ found: true, items: [vergeItem()] })
    const onImportVerge = vi.fn()
