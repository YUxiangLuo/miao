...
pub const ALL_SUBS_FAILED_KEEP_CACHE: &str = "所有订阅获取失败，继续使用缓存配置运行";
pub const ALL_SUBS_FAILED_RETRY: &str = "所有订阅获取失败，网络恢复后将自动重试";
pub const REFRESH_VALIDATION_FAILED: &str = "订阅刷新后的配置校验失败，继续使用缓存配置运行";
pub const REFRESH_FAILED_KEEP_CACHE: &str = "订阅刷新失败，继续使用缓存配置运行";
